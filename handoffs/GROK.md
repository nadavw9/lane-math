...
**Status:** DONE — draft PR open (MERGE HOLD Visual Scout + Games Lab)  
...
**Docs/handoff tip:** `1ffb7db` (`1ffb7db05392d9e7fcd192ea9dbed60857469d78`)  
**Writer:** Grok (Eng Lead sole writer)  
**Out of scope:** Base44, ChronosGlobe, solver/Director/levels/economy, `handoffs/CODEX.md`
...
Capture: Playwright via `tools/shot.mjs` pattern / one-off drive (`endLevelIntro` + `SHOT_SCREEN`-style ool/shop); phone viewport `393×852` @ DPR 3, `?sprites=1`, seed `docs/review/_hud-emblem-seed.json`.

## Gate findings

| Gate | Status | Finding |
|------|--------|---------|
| TX-P0-1 | **PASS (impl)** | CTA alone on gold; wait on separate felt pill above CTA inside panel |
| TX-P0-2 | **PASS (impl)** | Short title on plaque; pad under cartouche ≥8 (ships 20) |
| TX-P0-3 | **PASS (impl)** | Shop bottom anchored `pool.y - 56`; cubes readable under brass |
| Mascot idle | **PASS (impl)** | Breathe + sway; jump/droop unchanged; reduced-motion holds static |

## Unresolved risks

- Shop panel floors at **360** CSS for ornate 9-slice felt well — may still occlude **operator dials** (TX-P1-3 / phone-eye noise class) while clearing cube tops. Scout should reject only if cubes still kiss brass.
- Continuous idle redraw while companion is up (perf on low-end); gated off under reduced-motion / locked-out / enter tween.
- Stash `stash@{0}` had prior WIP on academy branch; applied code only after verify vs `cce5602` master (renderer tree identical to stash base `c6f63ad` for these files). Did **not** overwrite before-evidence PNGs from stash.
...
Please review **impl commit `dff2b98`** (not a chasing tip SHA) for TX-P0-1/2/3 layout contracts + mascot idle/reduced-motion. Visual Scout + Games Lab: MERGE HOLD until stills `49`/`50`/`51` pass readable-box SHIP gate. Do not merge this draft PR.

---

## Prior Track A (merged)

Academy reprice + star gates landed via PR #28 @ `cce5602`. See git history / closed #28 for economy ladder + gates.
