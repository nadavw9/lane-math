...
describe("first-session FTUE cue schedule", () => {
  it("walks the 1-01 action cues", () => {
    expect(ftueCue("1-01", 0, null, null)).toEqual({ line: "Tap a number.", pulse: "pool" });
    expect(ftueCue("1-01", 0, 2, null)).toEqual({ line: "Choose a sign.", pulse: "minus" });
    expect(ftueCue("1-01", 0, 2, "+")).toEqual({ line: "Make the target.", pulse: "queue" });
  });
  ...
  it("introduces the World 1 beats and World 2 multiply once", () => {
    ...
    expect(ftueCue("2-01", 0, null, null)).toEqual({ line: "New sign: multiply.", pulse: "multiply" });
  });
